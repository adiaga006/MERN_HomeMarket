...
const Signup = () => {
  ...
  const [signedUp, setSignedUp] = useState(false); // Thêm biến trạng thái
  ...
  const formSubmit = async () => {
    ...
    try {
      ...
      if (responseData.error) {
        ...
        setSignedUp(true); // Cập nhật biến trạng thái khi đăng ký thành công
        setData({
      ...
      //setData(prevData => ({ ...prevData, email: data.email }));
    } catch (error) {
      console.log(error);
    }
  };
  ...
  return (
    ...
    {/* Hiển thị ConfirmSignup nếu đã đăng ký thành công */}
